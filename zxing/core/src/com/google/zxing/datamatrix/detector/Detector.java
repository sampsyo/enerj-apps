...
public final class Detector {
  ...
  private static BitMatrix sampleGrid(BitMatrix image,
    ...
    return sampler.sampleGrid(
        ...
        0.0f,
        dimension,
        (@Precise float)topLeft.getX(),
        (@Precise float)topLeft.getY(),
        topRightX,
        topRightY,
        (@Precise float)bottomRight.getX(),
        (@Precise float)bottomRight.getY(),
        (@Precise float)bottomLeft.getX(),
        (@Precise float)bottomLeft.getY());
  }
  ...
  private ResultPointsAndTransitions transitionsBetween(ResultPoint from, ResultPoint to) {
    ...
    int toX = (int) to.getX();
    int toY = (int) to.getY();
    boolean steep = Endorsements.endorse(ApproxMath.abs(toY - fromY) > ApproxMath.abs(toX - fromX));
    if (steep) {
      int temp = fromX;
      fromX = fromY;
      fromY = temp;
    ...
    @Approx int dx = ApproxMath.abs(toX - fromX);
    @Approx int dy = ApproxMath.abs(toY - fromY);
    @Approx int error = -dx >> 1;
    int ystep = Endorsements.endorse(fromY < toY) ? 1 : -1;
    int xstep = Endorsements.endorse(fromX < toX) ? 1 : -1;
    int transitions = 0;
    @Approx boolean inBlack = image.get(steep ? fromY : fromX, steep ? fromX : fromY);
    for (int x = Endorsements.endorse(fromX), y = Endorsements.endorse(fromY); x != Endorsements.endorse(toX); x += xstep) { /* EnerJ TODO */
      @Approx boolean isBlack = image.get(steep ? y : x, steep ? x : y);
      if (Endorsements.endorse(isBlack != inBlack)) {
        transitions++;
        inBlack = isBlack;
      }
      error += dy;
      if (Endorsements.endorse(error) > 0) {
        if (y == Endorsements.endorse(toY)) {
          break;
        }
        y += ystep;
        error -= dx;
