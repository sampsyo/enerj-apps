...
public final class MonochromeRectangleDetector {
  ...
  private int[] blackWhiteRange(int fixedDimension, int maxWhiteRun, int minDim, int maxDim,
    ...
    int start = center;
    while (start >= minDim) {
      if (horizontal ? Endorsements.endorse(image.get(start, fixedDimension)) : Endorsements.endorse(image.get(fixedDimension, start))) {
        start--;
      } else {
    ...
    int end = center;
    while (end < maxDim) {
      if (horizontal ? Endorsements.endorse(image.get(end, fixedDimension)) : Endorsements.endorse(image.get(fixedDimension, end))) {
        end++;
      } else {
