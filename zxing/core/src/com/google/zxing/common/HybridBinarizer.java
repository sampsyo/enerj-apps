...
public final class HybridBinarizer extends GlobalHistogramBinarizer {
  ...
  private void binarizeEntireImage() throws NotFoundException {
    if (matrix == null) {
      ...
      if (source.getWidth() >= MINIMUM_DIMENSION && source.getHeight() >= MINIMUM_DIMENSION) {
        @Approx byte[] luminances = source.getMatrix();
        int width = source.getWidth();
        int height = source.getHeight();
        int subWidth = width >> 3;
        int subHeight = height >> 3;
        @Approx int[][] blackPoints = calculateBlackPoints(luminances, subWidth, subHeight, width);
  ...
  private static void calculateThresholdForBlock(@Approx byte[] luminances, int subWidth, int subHeight,
      int stride, @Approx int[][] blackPoints, BitMatrix matrix) {
    for (int y = 0; y < subHeight; y++) {
      for (int x = 0; x < subWidth; x++) {
        ...
        int sum = 0;
        for (int z = -2; z <= 2; z++) {
          @Approx int[] blackRow = blackPoints[top + z];
          sum += blackRow[left - 2];
          sum += blackRow[left - 1];
          sum += blackRow[left];
          sum += blackRow[left + 1];
  ...
  private static void threshold8x8Block(@Approx byte[] luminances, int xoffset, int yoffset, int threshold,
      int stride, BitMatrix matrix) {
    for (int y = 0; y < 8; y++) {
      int offset = (yoffset + y) * stride + xoffset;
      for (int x = 0; x < 8; x++) {
        int pixel = Endorsements.endorse(luminances[offset + x] & 0xff);
        if (pixel < threshold) {
  ...
  private static @Approx int[][] calculateBlackPoints(@Approx byte[] luminances, int subWidth, int subHeight,
      int stride) {
    @Approx int[][] blackPoints = new @Approx int[subHeight][subWidth];
    for (int y = 0; y < subHeight; y++) {
      for (int x = 0; x < subWidth; x++) {
        int sum = 0;
        int min = 255;
        ...
        for (int yy = 0; yy < 8; yy++) {
          ...
          for (int xx = 0; xx < 8; xx++) {
            int pixel = Endorsements.endorse(luminances[offset + xx] & 0xff);
            sum += pixel;
            if (pixel < min) {
              min = pixel;
            }
