...
public final class ITFReader extends OneDReader {
  ...
  private static void decodeMiddle(BitArray row, int payloadStart, int payloadEnd,
    ...
    @Approx int[] counterDigitPair = new @Approx int[10];
    @Approx int[] counterBlack = new @Approx int[5];
    @Approx int[] counterWhite = new @Approx int[5];
    ...
    while (payloadStart < payloadEnd) {
      ...
      @Approx int bestMatch = decodeDigit(counterBlack);
      resultString.append((char) Endorsements.endorse('0' + bestMatch));
  ...
  private void validateQuietZone(BitArray row, int startPattern) throws NotFoundException {
    ...
    for (int i = startPattern - 1; quietCount > 0 && i >= 0; i--) {
      if (Endorsements.endorse(row.get(i))) {
        break;
      }
      quietCount--;
    }
  ...
  private static int skipWhiteSpace(BitArray row) throws NotFoundException {
    ...
    while (endStart < width) {
      if (Endorsements.endorse(row.get(endStart))) {
        break;
      }
      endStart++;
    }
   ...
   */
  private static int[] findGuardPattern(BitArray row, int rowOffset, @Approx int[] pattern) throws NotFoundException {
    ...
    for (int x = rowOffset; x < width; x++) {
      ...
      if (Endorsements.endorse(pixel ^ isWhite)) {
        ...
        if (counterPosition == patternLength - 1) {
          ...
          patternStart += Endorsements.endorse(counters[0] + counters[1]);
   ...
   */
  private static int decodeDigit(@Approx int[] counters) throws NotFoundException {

    @Approx int bestVariance = MAX_AVG_VARIANCE; // worst variance we'll accept
    int bestMatch = -1;
    int max = PATTERNS.length;
    for (int i = 0; i < max; i++) {
      @Approx int[] pattern = (@Approx int[])PATTERNS[i];
