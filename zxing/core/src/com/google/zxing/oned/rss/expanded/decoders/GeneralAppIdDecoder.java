...
final class GeneralAppIdDecoder {
  ...
  private boolean isStillNumeric(int pos) {
    ...
    for(int i = pos; i < pos + 3; ++i) {
      if (Endorsements.endorse(this.information.get(i))) {
        return true;
      }
    }

    return Endorsements.endorse(this.information.get(pos + 3));
  }
  ...
  static int extractNumericValueFromBitArray(BitArray information, int pos, int bits) {
    ...
    for(int i = 0; i < bits; ++i) {
      if (Endorsements.endorse(information.get(pos + i))) {
        value |= (1 << (bits - i - 1));
      }
    }
  ...
  private boolean isAlphaTo646ToAlphaLatch(int pos) {
    ...
    for(int i = 0; i < 5 && i + pos < this.information.size; ++i){
      if(i == 2){
        if(!Endorsements.endorse(this.information.get(pos + 2))) {
          return false;
        }
      } else if(Endorsements.endorse(this.information.get(pos + i))) {
        return false;
      }
    }
  ...
  private boolean isNumericToAlphaNumericLatch(int pos) {
    ...
    for (int i = 0; i < 4 && i + pos < this.information.size; ++i) {
      if (Endorsements.endorse(this.information.get(pos + i))) {
        return false;
      }
    }
    return true;
