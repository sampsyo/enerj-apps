...
public final class Code93Reader extends OneDReader {
  ...
  public Result decodeRow(int rowNumber, BitArray row, Hashtable hints)
    ...
    do {
      ...
      @Approx int pattern = toPattern(counters);
      ...
      for (int i = 0; i < counters.length; i++) {
        nextStart += Endorsements.endorse(counters[i]);
      }
      // Read off white space
      while (nextStart < end && !row.get(nextStart)) {
        nextStart++;
    ...
    for (int i = rowOffset; i < width; i++) {
      ...
      if (Endorsements.endorse(pixel ^ isWhite)) {
        ...
        if (counterPosition == patternLength - 1) {
          if (Endorsements.endorse(toPattern(counters) == ASTERISK_ENCODING)) {
  ...
  private static @Approx int toPattern(@Approx int[] counters) {
    ...
    int pattern = 0;
    for (int i = 0; i < max; i++) {
      @Approx int scaledShifted = (counters[i] << INTEGER_MATH_SHIFT) * 9 / sum;
      @Approx int scaledUnshifted = scaledShifted >> INTEGER_MATH_SHIFT;
      if (Endorsements.endorse((scaledShifted & 0xFF) > 0x7F)) {
        scaledUnshifted++;
      }
      if (Endorsements.endorse(scaledUnshifted < 1 || scaledUnshifted > 4)) {
        return -1;
      }
      if ((i & 0x01) == 0) {
        for (int j = 0; j < Endorsements.endorse(scaledUnshifted); j++) {
