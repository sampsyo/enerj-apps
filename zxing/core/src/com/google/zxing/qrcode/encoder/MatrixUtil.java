...
public final class MatrixUtil {
  ...
  public static void embedTypeInfo(ErrorCorrectionLevel ecLevel, int maskPattern, ByteMatrix matrix)
    ...
    for (int i = 0; i < typeInfoBits.getSize(); ++i) {
      ...
      // "typeInfoBits".
      boolean bit = Endorsements.endorse(typeInfoBits.get(typeInfoBits.getSize() - 1 - i));
  ...
  public static void maybeEmbedVersionInfo(int version, ByteMatrix matrix) throws WriterException {
    ...
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 3; ++j) {
        ...
        boolean bit = Endorsements.endorse(versionInfoBits.get(bitIndex));
        bitIndex--;
        // Left bottom corner.
      ...
      while (y >= 0 && y < matrix.getHeight()) {
        for (int i = 0; i < 2; ++i) {
          ...
          if (bitIndex < dataBits.getSize()) {
            bit = Endorsements.endorse(dataBits.get(bitIndex));
            ++bitIndex;
          } else {
